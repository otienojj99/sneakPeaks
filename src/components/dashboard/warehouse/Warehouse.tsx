...
interface WarehouseListProps {
  ...
  onUpdateFilters: (filters: Partial<WarehouseFilters>) => void;
  onRefresh: () => void;
...
const WarehouseList = ({
  ...
  const toggleSelectAll = () => {
    if (selectedIds.length === warehouse.length) {
      setSelectedIds([]);
    } else {
      setSelectedIds(warehouse.map((c) => c.id));
    }
  };
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-gray-900">
                {warehouse.filter((c) => c.is_active).length}
              </p>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-gray-900">
                {warehouse.filter((c) => !c.is_primary).length}
              </p>
              <p className="text-xs text-gray-500">Top Warehouse</p>
            </div>
          </div>
        </div>
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-2xl font-bold text-gray-900">
                {warehouse.reduce((sum, c) => sum + (c.id || 0), 0)}
              </p>
                ...
                <thead>
                  <tr className="border-b border-gray-100 bg-gray-50/50">
                    <th className="text-left py-3.5 px-4 w-12">
                      ...
                      >
                        {selectedIds.length === warehouse.length &&
                        warehouse.length > 0 ? (
                          <FiCheckSquare
                            size={18}
                  ...
                  {warehouse.map((warehouses) => (
                    <tr
                      ...
                      className={`group hover:bg-gray-50/80 transition-colors ${
                        selectedIds.includes(warehouses.id)
                          ? "bg-emerald-50/40"
                          : ""
                      }`}
                    >
                      ...
                      <td className="py-3.5 px-4">
                        <button
                          onClick={() => toggleSelect(warehouses.id)}
                          className="text-gray-400 hover:text-gray-600 transition-colors"
                        >
                          {selectedIds.includes(warehouses.id) ? (
                            <FiCheckSquare
                      ...
                      <td className="py-3.5 px-4">
                        ...
                        >
                          ...
                          />
                          {warehouses.is_active ? "Active" : "Inactive"}
                        </span>
                      </td>
                      ...
                      <td className="py-3.5 px-4 hidden xl:table-cell">
                        <span className="text-sm text-gray-500">
                          {formatDate(warehouses.created_at)}
                        </span>
                      </td>
                          ...
                          {openActionId === warehouses.id && (
                            <>
                              ...
                              <div className="absolute right-0 top-full mt-1 z-20 bg-white rounded-xl border border-gray-200 shadow-xl py-1.5 w-44">
                                ...
                                <button
                                  onClick={() => handleDeleteClick(warehouses)}

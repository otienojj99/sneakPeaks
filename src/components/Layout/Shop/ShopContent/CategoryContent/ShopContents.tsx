...
export default function ShopContents() {
  ...
  useEffect(() => {
    fetchCategories();
    fetchTree();
  }, [fetchTree, fetchCategories]);

  useEffect(() => {
    console.log("🌳 categoryTree raw:", categoryTree);
    console.log("🌳 tree length:", categoryTree.length);
    categoryTree.forEach((c) => {
      console.log(
        `   → ${c.name} (id: ${c.id}) children:`,
        c.children?.length ?? "none",
        c.children,
      );
    });
  });
      ...
      <div className="flex flex-col lg:flex-row lg:items-start gap-8">
        ...
        >
          {categoriesLoading && categoryTree.length === 0 ? (
            ...
            <SidebarFilters
              categories={categoryTree as unknown as Category[]}

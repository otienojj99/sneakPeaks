// src/components/dashboard/suppliers/SupplierList.tsx
...
interface SupplierListProps {
  ...
  onUpdateFilters: (filters: Partial<SupplierFilters>) => void;
  onRefresh: () => void;
  onEdit: (supplier: Supplier) => void;
  onDelete: (id: number) => void;
  onBulkDelete: (ids: number[]) => void;
  onBulkStatusChange: (ids: number[], status: boolean) => void;
  onBulkPreferenceChange?: (ids: number[], preferred: boolean) => void;
}
  ...
  return (
    <>
        ...
        <div className="bg-white rounded-xl border border-gray-200 p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">High Rated (4+)</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm">
        <div className="p-4 flex flex-col sm:flex-row items-stretch sm:items-center gap-3 flex-wrap">
          {/* Search */}
          <form
          ...
          <select
            ...
            className="px-4 py-2.5 text-sm rounded-xl border border-gray-200 bg-gray-50 focus:bg-white focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-400 transition-all min-w-[120px]"
          >
          ...
          {/* Sort */}
          <select
            value={`${filters.sort_by || "company_name"}_${filters.sort_dir || "asc"}`}
            onChange={(e) => {
              const [field, direction] = e.target.value.split("_") as [
                "company_name" | "rating" | "created_at",
                "asc" | "desc",
              ];
              onUpdateFilters({ sort_by: field, sort_dir: direction });
            }}
          ...
          >
            ...
            <option value="company_name_desc">Name Z-A</option>
        ...
        {selectedIds.length > 0 && (
          <div className="px-4 py-3 bg-emerald-50 border-t border-emerald-100 flex flex-wrap items-center gap-3">
            ...
            {onBulkPreferenceChange && (
              <>
                <button
                  onClick={() => onBulkPreferenceChange(selectedIds, true)}
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-white border border-amber-200 text-amber-700 rounded-lg hover:bg-amber-50 transition-colors"
                >
                  <FiStar size={14} />
                  Mark Preferred
                </button>
                <button
                  onClick={() => onBulkPreferenceChange(selectedIds, false)}
                  className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-white border border-gray-200 text-gray-600 rounded-lg hover:bg-gray-100 transition-colors"
                >
                  <FiStar size={14} />
                  Unmark Preferred
        ...
        {isLoading ? (
          ...
          <div className="py-20 text-center">
            <div className="w-20 h-20 bg-gray-100 rounded-2xl flex items-center justify-center mx-auto mb-5">
              <FiGlobe className="text-gray-300" size={32} />
            </div>
